...
def chitu_calculate_crc(contents, seed):
...
def chitu_xor_block(r0, r1, block_number, block_size, file_key):
...
def chituf_encrypt_file(input, output_file, file_length):
    ...
    for block_number in range(0, block_count):
        ...
        chitu_xor_block(block_array, block_array, block_number, block_size, file_key)
        for n in range (0, block_size):
            input_file[block_offset + n] = block_array[n]

        # update the expected CRC value.
        xor_crc = chitu_calculate_crc(block_array, xor_crc)
...
def chituf_encrypt(source, output_directory):
    firmware = open(source, "rb")
    update = open(output_directory + '/update.cbd', "wb")
    length = os.path.getsize(source)

    chituf_encrypt_file(firmware, update, length)
...
def lerdge_encrypt(source, output_directory, target):
    ...
    # Copy over the lerdge crypt functions
    #chituf_encrypt_file(firmware, update, length)
...
# Main function
parser = argparse.ArgumentParser("Firmware Encryption Script Manager")
parser.add_argument("-s", "--sourceFile", help="source file.")
parser.add_argument("-o", "--outputDirectory", help="output directory.")
parser.add_argument("-t", "--targetEncryption", help="target encryption method to use.")

args = parser.parse_args()

print "Encrytping " + args.sourceFile + " " + "to " + args.outputDirectory + " for system " + args.targetEncryption
if args.targetEncryption == "CHITUF":
    chituf_encrypt(args.sourceFile, args.outputDirectory)
elif args.targetEncryption == "LERDGEX":
    lerdge_encrypt(args.sourceFile, args.outputDirectory, '/Lerdge_X_System_Firmware.bin')
elif args.targetEncryption == "LERDGEK":
    lerdge_encrypt(args.sourceFile, args.outputDirectory, '/Lerdge_K_System_Firmware.bin')
